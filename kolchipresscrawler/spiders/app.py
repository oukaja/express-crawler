...
from kolchipresscrawler.items import ArticleItem
...
class AppSpider(Spider):
    ...
    def parse(self, response):
        dn = self.data[str(urlparse(response.url).hostname).split('.')[1]]
        slidespath = dn["slid"]
    ...
    def parse_links(self, response):
        dn = response.meta['dn']
        item = ArticleItem()
        item["journal"] = str(urlparse(response.url).hostname).split('.')[1]
